      ...
      <main className="max-w-5xl mx-auto">
            ...
            {rawHtml && (
              <>
                <button
                  onClick={fetchEAF}
                  disabled={loading}
                  className="flex items-center justify-center gap-2 bg-card hover:bg-accent text-muted-foreground font-bold py-2 px-4 rounded-xl border border-border transition-all disabled:opacity-50 text-xs active:scale-95 shadow-sm"
                >
        ...
        {!rawHtml && !loading ? (
          <div className="bg-card rounded-2xl border border-border p-12 flex flex-col items-center justify-center text-center shadow-sm">
            <div className="bg-accent p-4 rounded-full mb-6 border border-border">
              <FileText className="h-8 w-8 text-muted-foreground" />
            </div>
            <h3 className="text-lg font-bold text-foreground">Certificate of Matriculation</h3>
            <p className="text-muted-foreground text-xs max-w-xs mx-auto mt-2 mb-8 font-medium leading-relaxed">
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
            ...
            <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden min-h-[600px]">
              <iframe
                srcDoc={rawHtml}
